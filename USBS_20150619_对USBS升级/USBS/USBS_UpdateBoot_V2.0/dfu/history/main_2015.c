...
{ /* Test if user code is programmed starting from address 0x8003000 */
	...
	{ /* Jump to user application */
		...
		//delay_ms(600);
		// 读取加密信息
//		STMFLASH_Read(0X08004C00, (uint16_t*)passwd, 1024/2);
//		for(addr=0; addr<128; addr+=4) // 加密已经存在,最后 1K数据不擦除
//		{
...
//		}
		...
		//for(addr=0; addr<(19*1024); addr+=1024) // 加密已经存在,最后 1K数据不擦除
		{
...
//	led = clock+40; // 0.04s
	...
	ledObj->LED2ON(0);
	ledObj->LED1ON(0);
	lprintf("boot_reset(0x%08X|0x%08X)...\r\n", ApplicationAddress, (*(__IO uint32_t*)ApplicationAddress));
	boot_reset();
}
...
//	uint32_t addr=0;
	...
	//delay_ms(3000);
	//for(clock=0; clock<2000; clock+=10)
	for(clock=0; clock<500; clock+=10)
	{
		delay_ms(10);
		if((clock+30) <= led)
		{
