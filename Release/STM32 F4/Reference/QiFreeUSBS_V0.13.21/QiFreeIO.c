...
static void QiFree_led_Init(QiFreeIO *io)
{
	...
	RCC_AHB1PeriphClockCmd(io->RCC_APB2Periph, ENABLE);	 //使能端口时钟
	
	GPIO_InitStructure.GPIO_Pin = io->Pin;				 //端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; 		 
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
	GPIO_Init(io->GPIOx, &GPIO_InitStructure);
	//GPIO_SetBits(io->GPIOx, io->Pin);
...
static void QiFree_led_default(uint32_t RCC_APB2Periph, GPIO_TypeDef* GPIOx, uint16_t PortVal, uint16_t Enable)
{
	int i=0;
	GPIO_InitTypeDef  GPIO_InitStructure;
	RCC_AHB1PeriphClockCmd(RCC_APB2Periph, ENABLE);	   //使能端口时钟
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
...
static void QiFree_key_Init(QiFreeIO *io)
{
	GPIO_InitTypeDef  GPIO_InitStructure;
	if(0==io->GPIOx) return; // 空的配置io->GPIOx
	RCC_AHB1PeriphClockCmd(io->RCC_APB2Periph, ENABLE);	 //使能PA,PD端口时钟
...
{
	uint8_t down = 0;
	if(0==io->GPIOx) return 1; // 空的配置io->GPIOx
