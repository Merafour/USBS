...
typedef struct{
	const uint32_t *sp1;
	//reset_func reset1;
	const uint32_t reset1;
	uint32_t ConfigExamples[(0x0E8)/4];
	uint8_t _version[0x10];
	const uint32_t *sp2;
	//reset_func reset2;
	const uint32_t reset2;
	uint8_t Reserve[0xF8];
}my_section;
extern const uint32_t __initial_sp;
extern void Reset_Handler(void);

__attribute__ ((section (".config"))) const my_section _my_section __attribute__((used))={
	&__initial_sp,
	((uint32_t)(&CodeCopyToRAM)-0x20000000+0x08000000),
	{
       0x01010000, // default //0K	| swd| erase| flashH| flashL|
	   //0x00010100, // default //256K	| swd| erase| flashH| flashL|
	   0x12,0x12,0x12,0x12,0x12,0x12,0x12,0x12, //mtext 
